 ...
 *
 * Flexigrid-S: static table extensions by Henry Rogers. 
 * Available for use with or without attribution under the MIT license.
 */
(function ($)
{
	$.addFlex = function (t, p)
	{
		...
		{ //apply default properties
			...
			showTableToggleBtn: true, //show or hide table toggle button
			hideOnSubmit: true,
			autoload: true,
			blockOpacity: 0.5,
			preProcess: false,
			...
			$(t).find('tr').each(
				function ()
				{
					...
					{				
						var row = {};
						row.id = $(this).attr('id');
						row.id = row.id.substring(row.id.indexOf('_') + 1);
						row.cell = [];
						$(this).find('td').each(
							function ()
							{
								row.cell.push($(this).html());
							}
						);
						d.rows.push(row);
					}
		...
		var g = {
					...
					$.each(
						...
						function (i, row)
						{
							...
							{ //handle if grid has no headers
								for (idx = 0; idx < row.cell.length; idx+=1)
								{
									var td = document.createElement('td');
									// If the json elements aren't named (which is typical), use numeric order
									if (typeof row.cell[idx] !== "undefined")
			...
			{ //get latest data
					...
					$.ajax(
						{
							...
							{
								data.meta.page = p.newp;
								g.d = p.parseData(data);
								g.dataset = g.d;
								g.addData(g.d);
							},
				...
				if (p.params)
				{
					for (pi = 0; pi < p.params.length; pi+=1)
					{
		...
		{ //create model if any
			...
			var tr = document.createElement('tr');
			var i;
			for (i = 0; i < p.colModel.length; i+=1)
			{
				var cm = p.colModel[i];
				...
				if (cm.filterable)
				{
					...
					if (p.searchcolumnindex === '' && cm.isdefault)
					{
						p.searchcolumnindex = i;
					}
				}
		...
		g.sDiv = document.createElement('div'); //create search field
		...
		if (p.buttons)
		{
			...
			var j;
			for (j = 0; j < p.buttons.length; j+=1)
			{
				var btn = p.buttons[j];
				if (!btn.separator)
				{
		...
		var theadfirst = $('thead:first', t).get(0);
		if (theadfirst)
		{
			$(g.hTable).append(theadfirst);
		}
		theadfirst = null;
		var ci = 0;
		if (!p.colModel)
			...
			function ()
			{
				...
				$(this).empty().append(thdiv).removeAttr('width').mousedown(
					...
					function ()
					{
						...
						if (p.usesort && !g.colCopy && !g.colresize && $(this).attr('abbr'))
						{
							if ($(this).attr('abbr') !== p.sortname)
							{
		...
		if (cdcol !== null)
		{
			...
			$('thead tr:first th', g.hDiv).each(
				function ()
				{
					...
					if ($.browser.msie && $.browser.version < 7.0)
					{
						g.fixHeight(); //$(g.gDiv).height() - not relevant
						$(cgDiv).hover(
							function ()
							{
								g.fixHeight();
		...
		if (p.usepager)
		{
			...
			if (p.useRp)
			{
				var opt = '',
					sel = '';
				var nx = 0;
				for (nx = 0; nx < p.rpOptions.length; nx+=1)
				{
			...
			if (p.searchitems)
			{
				...
				var sopt = '',
					ssel = '';
				var si = 0;
				for (si = 0; si < sitems.length; si+=1)
				{
					if (p.qtype === '' && sitems[si].isdefault === true)
					{
						p.qtype = sitems[si].name;
					...
					sopt += "<option value='" + sitems[si].name + "' " + ssel + " >" + sitems[si].display + "&nbsp;&nbsp;</option>";
				}
				if (p.qtype === '')
				{
					p.qtype = sitems[0].name;
				}
				$(g.sDiv).append('<div class="sDiv2">' + p.findtext + ' <input type="text" value="' + p.query + '" size="30" name="q" class="qsbox" /> ' + ' <select name="qtype">' + sopt + '</select><input type="button" value="Search" class="button" /> <input type="button" value="Clear" class="button" /></div>');
				//Split into separate selectors because of bug in jQuery 1.3.2
				$('input[name=query],select[name=qtype]', g.sDiv).keydown(
					function (e)
					{
						if (e.keyCode === 13)
						{
				...
				$('select[name=qtype]', g.sDiv).keydown(
					function (e)
					{
						if (e.keyCode === 13)
						{
				...
				$('input[value=Search]',g.sDiv).click(
				   function ()
				   {
					   g.doSearch();
				   }
		...
		if ($('th', g.hDiv).length)
		{
			...
			$(g.nBtn).addClass('nBtn').html('<div></div>').attr('title', 'Hide/Show Columns').click(
				function ()
				{
					$(g.nDiv).toggle();
					return true;
